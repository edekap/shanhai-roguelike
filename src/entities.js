...
const BOSS_IMG_PATHS = {
  ...
  1: 'assets/bosses/bifang_v2_wide.jpg',    // 毕方
  2: 'assets/bosses/xiangliu_v2_wide(1).jpg', // 相柳
  3: 'assets/bosses/zhuyan_v2_wide.jpg',    // 朱厌
  4: 'assets/bosses/zhulong_v2_wide.jpg',   // 烛龙
  5: 'assets/bosses/taotie_v2_wide.jpg',    // 饕餮
  6: 'assets/bosses/yingzhao_v2_wide.jpg',  // 英招
  7: 'assets/bosses/jimeng_v2_wide.jpg',    // 计蒙
  8: 'assets/bosses/qiongqi_v5_mythic.jpg', // 穷奇
  9: null   // 刑天（最终Boss，专用图通过 loadXingtianImages 加载）
};
// 攻击帧路径（-attack_wide.jpg / -attack_mythic.jpg）
const BOSS_IMG_ATTACK_PATHS = {
  0: 'assets/bosses/jiuweihu_v2-attack_wide.jpg',
  1: 'assets/bosses/bifang_v2-attack_wide.jpg',
  2: 'assets/bosses/xiangliu_v2-attack_wide(1).jpg',
  3: 'assets/bosses/zhuyan_v2-attack_wide.jpg',
  4: 'assets/bosses/zhulong_v2-attack_wide.jpg',
  5: 'assets/bosses/taotie_v2-attack_wide.jpg',
  6: 'assets/bosses/yingzhao_v2-attack_wide.jpg',
  7: 'assets/bosses/jimeng_v2-attack_wide.jpg',
  8: 'assets/bosses/qiongqi_v5-attack_mythic.jpg'
};
// 帧能力表：所有Boss均提供 idle + attack 两帧（jpg已统一生成）
const BOSS_FRAME_CAPABILITY = {
  0: {attack:true,  attack2:false, move:false}, // 九尾狐
  1: {attack:true,  attack2:false, move:false}, // 毕方
  2: {attack:true,  attack2:false, move:false}, // 相柳
  3: {attack:true,  attack2:false, move:false}, // 朱厌
  4: {attack:true,  attack2:false, move:false}, // 烛龙
  5: {attack:true,  attack2:false, move:false}, // 饕餮
  6: {attack:true,  attack2:false, move:false}, // 英招
  7: {attack:true,  attack2:false, move:false}, // 计蒙
  8: {attack:true,  attack2:false, move:false}, // 穷奇
  9: {attack:true,  attack2:false, move:false}  // 刑天（专用图，通过 loadXingtianImages 加载）
};
...
function _makeBossTransparent(img){
  ...
  try{
    ...
    // 跨域或其它异常，回退到原jpg（会有白底，但不影响游戏运行）
    result = img;
  }
  _bossTransparentCache.set(img, result);
  return result;
}
const BOSS_IMAGES = {};
...
// jpg加载后通过 _makeBossTransparent 去除白底，得到透明背景canvas
function loadBossImagesForIdx(idx){
  ...
  const idlePath = BOSS_IMG_PATHS[idx];
  const atkPath = BOSS_IMG_ATTACK_PATHS[idx] || null;
...
function pushFloatingText(x,y,text,color,life=1,size,type='normal'){
  ...
  const ox=rand(-15,15);
  const oy=rand(-10,5);
  // 类型决定视觉风格：normal普通/crit暴击红字放大/pierce穿透紫字/combo连击火焰拖尾/heal治疗绿字/exec处决金字
  const t={x:x+ox,y:y+oy,text,color,life,maxLife:life,size:size||0,type,
    // 出生时间戳，用于计算动画进度（震颤/缩放）
    born:_NOW,
    // 暴击横向偏移初速度（让暴击数字左右抖一下）
    vx:type==='crit'?rand(-30,30):0,
    // 连击火焰拖尾粒子计数
    trail:type==='combo'?[]:null
  };
...
// 伤害飘字专用快捷函数：根据伤害类型自动选颜色和样式
// dmgType: 'normal'|'crit'|'pierce'|'exec'
function pushDamageText(x,y,dmg,dmgType='normal'){
  const d=Math.ceil(dmg);
  if(dmgType==='crit'){
    // 暴击：红字放大 + 火焰描边
    pushFloatingText(x,y,`${d}`,'#ff4d4d',0.9,26,'crit');
  }else if(dmgType==='pierce'){
    // 穿透：紫字带穿心效果
    pushFloatingText(x,y,`${d}`,'#bc8cff',0.85,20,'pierce');
  }else if(dmgType==='exec'){
    // 处决：金色"处决!"
    pushFloatingText(x,y,`处决 ${d}`,'#ffd700',1.0,24,'exec');
  }else{
    // 普通伤害：白字
    pushFloatingText(x,y,`${d}`,'#ffffff',0.7,15,'normal');
  }
}
// 全屏Toast提示（用于菜单界面）
let toastTimer=null;
...
function updateFloatingTexts(dt){
  for(const t of floatingTexts){
    ...
    // 连击数字留下火焰拖尾
    if(t.type==='combo' && t.trail && Math.random()<0.5){
      t.trail.push({x:t.x+rand(-8,8),y:t.y+rand(-4,4),life:0.3,maxLife:0.3,size:rand(3,6)});
      if(t.trail.length>8)t.trail.shift();
    }
    if(t.trail){
      for(const p of t.trail){p.life-=dt;}
  ...
  for(const t of floatingTexts){
    ...
    if(t.trail){
      for(const p of t.trail){
        ...
        ctx.fillStyle=pa>0.5?'#ff8c42':'#ffd700';
        ctx.beginPath();ctx.arc(p.x,p.y,p.size*pa,0,Math.PI*2);ctx.fill();
      }
      ctx.globalAlpha=Math.min(1,t.life*2);
    }
    ctx.fillStyle=t.color;
    const _size=t.size||14;
    // 出生0.15秒内有放大入场动画（除普通伤害外）
    let scale=1;
    if(t.type!=='normal' && t.type!=='heal' && age<150){
      const p=age/150; // 0-1
      scale=0.5+1.2*p-0.7*p*p; // 0.5→1.2→1.0 弹性曲线
    }
    // 暴击额外震颤（出生后0.2秒内左右抖）
    let shakeX=0;
    if(t.type==='crit' && age<200){
      shakeX=Math.sin(age*0.08)*2;
    }
    ctx.font=`bold ${Math.round(_size*scale)}px Arial`;
    ...
    // 暴击/处决 描边强化
    if(t.type==='crit' || t.type==='exec'){
      ctx.lineWidth=3;
      ctx.strokeStyle='rgba(0,0,0,0.8)';
      ctx.strokeText(t.text,t.x+shakeX,t.y);
    }
    ctx.fillText(t.text,t.x+shakeX,t.y);
    ctx.shadowBlur=0;
  }
  ctx.globalAlpha=1;
}
...
class Enemy {
  ...
  takeDamage(dmg,bullet){
    ...
    if(player&&player.executeThreshold&&this.health>0&&this.health<=this.maxHealth*player.executeThreshold&&!this.secondBar){
      this.health=0; spawnParticles(this.x,this.y,'#ff4444',20);
      // 处决使用专属金色"处决!"飘字
      pushDamageText(this.x,this.y-15,dmg,'exec');
      // 处决击杀：轻微hit-stop强化爽感
    ...
    if(bullet && !bullet._isChain){
      if(bullet.isCrit){
        pushDamageText(this.x,this.y-30,dmg,'crit');
      }else if(bullet.pierce>0){
        pushDamageText(this.x,this.y-30,dmg,'pierce');
      }
      // 普通伤害有30%概率飘字（避免每枪都飘字过密）
      else if(Math.random()<0.3){
        pushDamageText(this.x,this.y-30,dmg,'normal');
      }
    }
    if(bullet){
      // 元素伤害可视化：不同颜色浮字
  ...
  die(killingBullet){
    this.alive=false;
    // Hit-stop分级：根据怪物等级触发不同长度的冻结，强化打击感
    // 普通怪(grunt/runner)不触发，避免频繁卡顿；精英/大型怪触发0.03-0.05秒
    if(typeof triggerHitStop==='function'){
      const _tier=this.tier||0;
      if(_tier>=3){ // giant/splitter等大型怪
        triggerHitStop(0.05);
      }else if(_tier===2){ // tank等中型怪
        triggerHitStop(0.03);
      }
    }
    // 死亡复盘：统计击杀数和经验
...
const BOSS_LORE = {
  ...
  2: { // 相柳
    ...
    weakness:'⚠️ 弱点：毒辐从Boss中心环绕发出，注意观察光束顺序找出间隙。毒藤缠绕区域高伤，立即离开。',
    skills:['九头毒液','毒藤缠绕','全图毒辐']
  },
  3: { // 朱厌
    story:'小次之山，有兽焉，其状如猿，而白首赤足，名曰朱厌，见则大兵。朱厌性烈好斗，一怒则山崩地裂，巨石如雨。',
    weakness:'⚠️ 弱点：追踪践踏缓慢但持续，保持移动并利用间隙输出。震地冲击有预警圈，提前躲开。',
    skills:['巨石投掷','震地冲击','追踪践踏']
  },
  4: { // 烛龙
  ...
  6: { // 英招
    ...
    weakness:'⚠️ 弱点：冲刺后短暂停歇，趁其停滞时全力输出。龙卷风缓慢可侧向躲开。',
    skills:['风刃','龙卷旋风','三段冲刺']
  },
  7: { // 计蒙
...
class Boss {
  ...
  triggerQuarterHealthSkill(){
    ...
    if(idx===0){
      ...
      showWaveAnnounce('二阶段！','英招：疾风冲刺！三段冲刺后有停顿',true);
  ...
  draw(){
    ...
    if(this.isFinalBoss){
      ...
      // jpg图片：先经过透明化处理（白底→透明），再绘制
      // 2.2倍碰撞圈，确保Boss完整身体可见
      const imgS=s*2.2;
  ...
  // 刑天主体绘制：使用jpg图片（常态/攻击形态切换），加载后透明化处理，未加载时回退到程序化绘制
  drawXingtianBody(s,t){
    ...
    if(xtImg && xtImg.complete && xtImg.naturalWidth>0){
      // jpg透明化处理（首次调用处理，后续从缓存读取）
      const tXt=_makeBossTransparent(xtImg);
      ctx.save();
      ctx.scale(breath,breath);
      const imgS=s*2.2;
